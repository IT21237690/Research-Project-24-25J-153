...
import { FaMicrophone, FaRegStar, FaSpinner, FaStar, FaStarHalfAlt } from "react-icons/fa";

interface ApiResponse {
  final_fuency_score: number;
...
const VoiceAnswerContent = () => {
  ...
  const getStars = (score, color) => {
    const totalStars = 3;
    const filledStars = Math.floor(score * totalStars); // Full stars
    const hasHalfStar = score * totalStars - filledStars >= 0.5; // Half-star check
    ...
    return (
      <div className="flex items-center justify-center space-x-2 text-5xl">
        ...
        {Array(filledStars)
          .fill(null)
          .map((_, i) => (
            <FaStar key={`full-${i}`} className={`text-${color}-500`} />
          ))}
  
        {/* Half star */}
        {hasHalfStar && <FaStarHalfAlt className={`text-${color}-500`} />}
  ...
  return (
    <div className="bg-white/30 rounded-lg shadow-md p-5 w-full max-w-none mt-5 backdrop-blur-lg">
      ...
      {isLoading && (
        <div className="mt-6 flex flex-col items-center">
          ...
          <p className="text-5xl font-semibold text-black animate-bounce">
            Sending your recording...
          </p>

          <p className="text-md font-medium text-gray-700 animate-pulse">
            ...
            <div className="mb-4">
              ...
              <div className="flex items-center justify-center">
                {pronunciationScore <= 0 ? (
                  <FaStar className="text-green-500 text-5xl" />
                ) : pronunciationScore > 0 && pronunciationScore <= 0.5 ? (
                  <>
                    <FaStar className="text-green-500 text-5xl" />
                    <FaStar className="text-green-500 text-5xl" />
                  </>
    ...
    {pronunciationScore !== null && (
      <div className="mb-4">
        <p className="text-lg font-semibold">Pronunciation</p>
        {getStars(pronunciationScore, "green")}
      </div>
    )}
          {/* Mispronounced Words */}
