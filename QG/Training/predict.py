...
def run_inference():
    ...
    # --------------------------------------------------
    model_name = "t5-small"
    tokenizer = T5Tokenizer.from_pretrained(model_name, legacy=False)  # Fixes legacy warning
    ...
    saved_model_path = "/home/minidu-tissera/PycharmProjects/Research-Project/v11/QG/Models/custom_qg_model_v3.pt"
    model.load_state_dict(torch.load(saved_model_path, map_location="cpu", weights_only=True))  # Fixes torch.load warning
    model.eval()
    ...
    # 3. Example Inference
    # --------------------------------------------------
    passage = "I wear my raincoat and play in the puddles."
    print("\nPassage:", passage)  # Print the passage
